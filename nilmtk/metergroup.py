...
from .datastore.datastore import MAX_MEM_ALLOWANCE_IN_BYTES
from .elecmeter import ElecMeter, ElecMeterID
from .electric import Electric
from .measurement import AC_TYPES, LEVEL_NAMES, PHYSICAL_QUANTITIES_TO_AVERAGE
...
class MeterGroup(Electric):
    ...
    def import_metadata(self, store, elec_meters, appliances, building_id):
        ...
        if not elec_meters:
            warnings.warn(
                "Building {} has an empty 'elec_meters' object.".format(building_id.instance),
                RuntimeWarning,
            )
        if not appliances:
            warnings.warn(
                "Building {} has an empty 'appliances' list.".format(building_id.instance),
                RuntimeWarning,
            )
        ...
        meters_to_disable = [m for m in self.meters if isinstance(m, ElecMeter) and m.metadata.get("disabled")]
        ...
        * `{'dataset': 'redd', 'building': 3, 'type': 'toaster', 'instance': 2}`
        ...
        if isinstance(key, str):
            ...
            if isinstance(key.instance, tuple):
                ...
                metergroup_of_building = self.select(building=key.building, dataset=key.dataset)
        ...
        # find MeterGroup from list of ElecMeterIDs
            ...
            if n_meters_found > 1:
                raise Exception("{} meters found with instance == {}: {}".format(n_meters_found, key, meters_found))
    ...
    def values_for_appliance_metadata_key(self, key, only_consider_dominant_appliance=True):
        """
        Parameters
        ----------
        key : str
    ...
    def total_energy(self, **load_kwargs):
        ...
        **loader_kwargs : key word arguments for DataStore.load()
        ...
        meter_energies = self._collect_stats_on_all_meters(load_kwargs, "total_energy", full_results)
    ...
    def _collect_stats_on_all_meters(self, load_kwargs, func, full_results):
        ...
        for meter in self.meters:
            ...
            if full_results and len(self.meters) > 1 and not meter.store.all_sections_smaller_than_chunksize:
                warnings.warn(
    ...
    def dropout_rate(self, **load_kwargs):
        ...
        **loader_kwargs : key word arguments for DataStore.load()
        ...
        dropout_rates = self._collect_stats_on_all_meters(load_kwargs, "dropout_rate", full_results)
    ...
    def _check_kwargs_for_full_results_and_sections(self, load_kwargs):
        if load_kwargs.get("full_results") and "sections" not in load_kwargs and len(self.meters) > 1:
            raise RuntimeError(
    ...
    def available_ac_types(self, physical_quantity):
        ...
        """
        all_ac_types = [meter.available_ac_types(physical_quantity) for meter in self.meters]
    ...
    def available_physical_quantities(self):
        ...
        """
        all_physical_quants = [meter.available_physical_quantities() for meter in self.meters]
        return list(set(flatten_2d_list(all_physical_quants)))

    def energy_per_meter(self, per_period=None, mains=None, use_meter_labels=False, **load_kwargs):
        ...
        if mains is not None:
            energy_per_meter = self._energy_per_meter_with_remainder(energy_per_meter, mains, per_period, **load_kwargs)

        return energy_per_meter

    def _energy_per_meter_with_remainder(self, energy_per_meter, mains, per_period, **kwargs):
        ...
        mains_ac_types = mains.available_ac_types(["power", "energy", "cumulative energy"])
        ...
        if per_period is None:
            ...
            mains_energy = mains.average_energy_per_period(offset_alias=per_period, **kwargs)
    ...
    def plot(self, kind="separate lines", **kwargs):
        ...
        try:
            ...
            raise ValueError("'{}' not a valid setting for 'kind' parameter.".format(kind))
        return ax
    ...
    def _plot_energy_bar(self, ax=None, mains=None):
        ...
        ax
        """
        energy = self.energy_per_meter(mains=mains, per_period="D", use_meter_labels=True)

        energy.sort(ascending=False)

        # Plot
        ax = pd.DataFrame(energy).T.plot(kind="bar", stacked=True, grid=True, edgecolor="none", legend=False, width=2)
        ax.set_xticks([])
    ...
    def describe(self, compute_expensive_stats=True, **kwargs):
        ...
        if compute_expensive_stats:
            series["correlation_of_sum_of_submeters_with_mains"] = self.correlation_of_sum_of_submeters_with_mains(
                **kwargs
            )
            series["proportion_of_energy_submetered"] = self.proportion_of_energy_submetered(**kwargs)
            dropout_rates = self._collect_stats_on_all_meters(kwargs, "dropout_rate", False)
...
def combine_chunks_from_generators(index, columns, meters, kwargs):
    ...
    for meter in meters:
        ...
        columns_to_average = set(PHYSICAL_QUANTITIES_TO_AVERAGE).intersection(physical_quantities)
        if columns_to_average:
            ...
            columns_to_average_counter = columns_to_average_counter.add(counter_increment, fill_value=0)
