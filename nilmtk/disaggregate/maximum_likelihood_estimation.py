...
class MLE(Disaggregator):
	...
	powerNoise: int
		For the disaggregate_chunk method, minimum delta value of a event to be considered, otherwise is noise. 
	powerPair: int
	...
	def train(self, metergroup):
		...
		* Instance is initialised to 1. Use meter.instance to provide more information (TODO)
		...
		for meter in metergroup.meters: 
			for chunk in meter.power_series(): 
				...
				if chunk.empty: 
					...
					self.train_on_chunk(pd.DataFrame(chunk.resample(self.sample_period, how=self.sampling_method)),identifier)
	...
	def train_on_chunk(self, chunk, identifier): 
		"""
		Extracts features  from chunk, concatenates feature_train (onpower_train, offpower_train and duration_train) with new features and retrains feature models. 
		...
		* Disaggregates only the selected appliance.(TODO: Disaggregates many)
		...
		if len(edges) > 1: 
			...
			number_of_events = 0
			print "WARNING: No paired events found on this chunk. Is it thDelta high?"
		...
		if self.onpower['name'] == 'gmm': 
			...
			raise NameError('Name of the model for onpower unknown or not implemented')
		...
		if self.offpower['name'] == 'gmm':
			...
			raise NameError('Name of the model for offpower unknown or not implemented')
		...
		if self.duration['name'] == 'gmm':
			...
			raise NameError('Name of the model for duration unknown or not implemented')

		# UPDATE STATS: 
		stat_dict = {'appliance':identifier[0], 'instance':identifier[1], 'Nevents':number_of_events}
		instanceFound = False
		if len(self.stats) == 0: 
	...
	def disaggregate(self, mains, output_datastore):
		...
		for chunk in mains.power_series():
			dis_chunk = self.disaggregate_chunk(pd.DataFrame(chunk.resample(self.sample_period,how=self.sampling_method)))
	...
	def disaggregate_chunk(self, chunk):
		...
		* Disaggregates only the selected appliance.(TODO: Disaggregates many)
		...
		if not self.resistive: 
			...
			if units_mismatched: 
				stringError = self.appliance + " cannot be disaggregated. " + self.appliance+" is a non-resistive elementa and  units mismatches: disaggregated data is in " + str(self.units) + " and aggregated data is " + str(units)
		...
		for onevent in events[events.onpower==True].iterrows(): 
			...
			deltaOn = onevent[1][1]
			# windowning: 
			offevents = events[(events.offpower == True) & (events.index > onTime) & (events.index < onTime + timedelta(seconds=self.timeWindow))]
			# Filter paired events: 
			offevents = offevents[abs(deltaOn - offevents[column_name].abs()) < self.powerPair]
			...
			if not offevents.empty: 
				if self.onpower['name'] == 'norm': 
					...
					deltaOn = np.round(deltaOn)  	# Decimal values produce odd y values
				...
				for offevent in offevents.iterrows(): 
					...
					if self.offpower['name'] == 'norm': 
						...
						poff = np.exp(self.offpower['model'].score([deltaOff]))[0]
					elif self.offpower['name'] == 'poisson': 
						deltaOff = np.round(deltaOff)  	# Decimal values produce odd y values
					...
					if self.duration['name'] == 'norm': 
						pduration = self.duration['model'].pdf(duration.total_seconds())
					elif self.duration['name'] == 'gmm': 
						pduration = np.exp(self.duration['model'].score([duration.total_seconds()]))[0]
					elif self.duration['name'] == 'poisson': 
						seconds = np.round(duration.total_seconds())  	# Decimal values produce odd y values
					...
					detection_list.append({'likelihood': likelihood, 'onTime':onTime, 'offTime': offTime, 'deltaOn':deltaOn})
			else: 
				singleOnevent +=1
		...
		for i in range(len(detection_list)): 
			detections.loc[i] = [detection_list[i]['onTime'],detection_list[i]['offTime'], detection_list[i]['likelihood'], detection_list[i]['deltaOn']]
		...
		for row in detections.iterrows(): 
			...
			if ((dis_chunk[(dis_chunk.index >= onTime) & (dis_chunk.index < offTime)].sum().values[0]) == 0):
				...
				dis_chunk[(dis_chunk.index >= onTime) & (dis_chunk.index < offTime)] = deltaOn
	...
	def featuresHist(self, **kwargs):
		...
		## Evaluating score for: 
		# Onpower
		x = np.arange(bins_onpower.min(), bins_onpower.max()+np.diff(bins_onpower)[0], np.diff(bins_onpower)[0] / float(1000)).reshape(-1,1)
		if self.onpower['name'] == 'norm': 
		...
		# Offpower
		x = np.arange(bins_offpower.min(), bins_offpower.max()+np.diff(bins_offpower)[0], np.diff(bins_offpower)[0] / float(1000)).reshape(-1,1)
		if self.offpower['name'] == 'norm': 
		...
		# Duration
		x = np.arange(bins_duration.min(), bins_duration.max()+np.diff(bins_duration)[0], np.diff(bins_duration)[0] / float(1000)).reshape(-1,1)
		if self.duration['name'] == 'norm': 
