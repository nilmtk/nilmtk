...
class CO_1d(object):
    ...
    def test(self, test_mains, test_appliances):
        ...
        power_column = test_mains.columns[0]
        length_sequence = len(test_mains[power_column])
        states = np.zeros(length_sequence)
        residual_power = np.zeros(length_sequence)
        for i in range(length_sequence):
            [states[i], residual_power[i]] = find_nearest(sum_combination,
                test_mains[power_column])
