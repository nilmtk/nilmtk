    ...
    for house_id in houses:
        ...
        for y in years:
            ...
            if not meters_paths_csv:
                ...
                df_all_years = df_all_years.append(df_year, ignore_index=True, sort=False)
        # Change index to datetime format
        df_all_years["Date & Time"] = pd.to_datetime(df_all_years["Date & Time"], utc=True)
...
def _find_year(input_path, house_id):
    ...
    """
    year_names = [
        p for p in listdir(input_path + "/" + "Home" + house_id) if isdir(join(input_path + "/" + "Home" + house_id, p))
    ]
    return year_names
